...
class QAQualityStats(BaseModel):
    """问答质量与知识库调用统计（代理指标）"""
    ...
    kb_tool_calls: int
    kb_tool_success_calls: int
    kb_tool_success_rate: float
    top_kb_tools: list[dict]
    dislike_reasons_top: list[dict]
    quality_report_coverage_rate: float = 0.0
...
@dashboard.get("/stats/qa-quality", response_model=QAQualityStats)
async def get_qa_quality_stats(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_admin_user),
):
    """获取问答质量与知识库调用情况（Admin only）"""
    try:
        from src.storage.db.models import MessageFeedback, ToolCall, Message, Conversation
        ...
        satisfaction_rate = round((like_count / total_feedbacks * 100), 2) if total_feedbacks > 0 else 0.0

        # 知识库工具调用（约定前缀：query_）
        kb_tool_calls = db.query(func.count(ToolCall.id)).filter(ToolCall.tool_name.like("query_%")).scalar() or 0
        kb_tool_success_calls = (
            db.query(func.count(ToolCall.id))
            .filter(ToolCall.tool_name.like("query_%"), ToolCall.status == "success")
            .scalar()
            or 0
        )
        kb_tool_success_rate = round((kb_tool_success_calls / kb_tool_calls * 100), 2) if kb_tool_calls > 0 else 0.0

        top_kb_tools_rows = (
            db.query(ToolCall.tool_name, func.count(ToolCall.id).label("count"))
            .filter(ToolCall.tool_name.like("query_%"))
            .group_by(ToolCall.tool_name)
            .order_by(func.count(ToolCall.id).desc())
            .limit(8)
            .all()
        )
        top_kb_tools = [{"tool_name": name, "count": count} for name, count in top_kb_tools_rows]

        dislike_reasons_rows = (
            db.query(MessageFeedback.reason, func.count(MessageFeedback.id).label("count"))
            .filter(MessageFeedback.rating == "dislike", MessageFeedback.reason.isnot(None), MessageFeedback.reason != "")
            .group_by(MessageFeedback.reason)
            .order_by(func.count(MessageFeedback.id).desc())
            .limit(8)
            .all()
        )
        dislike_reasons_top = [{"reason": reason, "count": count} for reason, count in dislike_reasons_rows]

        # 质量闭环指标（来自 assistant message.extra_metadata）
        assistant_rows = (
        ...
        for msg, conv in assistant_rows:
            extra = msg.extra_metadata or {}
            quality = extra.get("quality_report", {})
            source_refs = extra.get("source_refs", [])
            contract_validation = extra.get("contract_validation", {})

            if quality:
                quality_covered += 1
                if str(quality.get("confidence_level", "")).lower() == "low":
                    low_confidence_count += 1
            if contract_validation and not contract_validation.get("passed", True):
            ...
            agent_bucket = agent_quality_stats.setdefault(
                conv.agent_id,
                {"total": 0, "quality_covered": 0, "low_confidence": 0, "contract_failed": 0},
            )
            agent_bucket["total"] = int(agent_bucket["total"]) + 1
            if quality:
                agent_bucket["quality_covered"] = int(agent_bucket["quality_covered"]) + 1
                if str(quality.get("confidence_level", "")).lower() == "low":
                    agent_bucket["low_confidence"] = int(agent_bucket["low_confidence"]) + 1
            if contract_validation and not contract_validation.get("passed", True):
                agent_bucket["contract_failed"] = int(agent_bucket["contract_failed"]) + 1

        quality_report_coverage_rate = round((quality_covered / total_assistant * 100),2) if total_assistant > 0 else 0.0
        ...
        for agent, bucket in agent_quality_stats.items():
            ...
            quality_by_agent.append(
                {
                    "agent_id": agent,
                    "quality_coverage_rate": round((int(bucket["quality_covered"]) / total * 100), 2) if total else 0.0,
                    "low_confidence_rate": round((int(bucket["low_confidence"]) / total * 100), 2) if total else 0.0,
                    "contract_fail_rate": round((int(bucket["contract_failed"]) / total * 100), 2) if total else 0.0,
                    "sample_count": total,
                }
            )
        quality_by_agent = sorted(quality_by_agent, key=lambda x: x["sample_count"], reverse=True)[:10]

        return QAQualityStats(
            total_feedbacks=total_feedbacks,
            like_count=like_count,
            dislike_count=dislike_count,
            satisfaction_rate=satisfaction_rate,
            kb_tool_calls=kb_tool_calls,
            kb_tool_success_calls=kb_tool_success_calls,
            kb_tool_success_rate=kb_tool_success_rate,
            top_kb_tools=top_kb_tools,
            dislike_reasons_top=dislike_reasons_top,
            quality_report_coverage_rate=quality_report_coverage_rate,
        ...
        logger.error(f"Error getting QA quality stats: {e}")
        logger.error(traceback.format_exc())
        raise HTTPException(status_code=500, detail=f"Failed to get QA quality stats: {str(e)}")
