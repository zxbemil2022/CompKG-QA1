...
/**
  ...
  getMyFeedbacks: (params = {}) => {
    ...
    if (params.rating && params.rating !== 'all') queryParams.append('rating', params.rating)
    if (params.agent_id) queryParams.append('agent_id', params.agent_id)
    return apiGet(`/api/chat/feedbacks?${queryParams.toString()}`)
  },
  ...
  /**
   * 获取问答质量与知识库调用情况
   * @returns {Promise<Object>} - 质量与调用统计
   */
  getQAQualityStats: () => {
    return apiAdminGet('/api/dashboard/stats/qa-quality')
  },
  /**
   * 批量获取所有统计数据（并行请求）
  ...
  getAllStats: async () => {
    try {
      const [basicStats, userStats, toolStats, knowledgeStats, agentStats, qaQualityStats] = await Promise.all([
        ...
        apiAdminGet('/api/dashboard/stats/qa-quality')
      ])
