...
function initGraph() {
  ...
  graphInstance = new Graph({
    ...
    node: {
      type: props.nodeStyleOptions.type || 'circle',
      style: {
      ...
      state: props.nodeStyleOptions.state || {
        hidden: { opacity: 0.2, 'label-text-opacity': 0.2 },
        focusCore: {
          opacity: 1,
          stroke: '#2563eb',
          lineWidth: 3,
    ...
    edge: {
      ...
      state: props.edgeStyleOptions.state || {
        hidden: { opacity: 0.12, 'label-text-opacity': 0 },
...
function bindEvents() {
  ...
  if (props.enableFocusNeighbor) {
    graphInstance.on('node:click', async (e) => {
      ...
      updates[clickedNodeId] = ['focusCore']
      Array.from(neighborSet).forEach(id => updates[id] = ['focusNeighbor'])
...
async function focusNode(id) {
  ...
  updates[id] = ['focusCore']
  Array.from(neighborSet).forEach(nid => updates[nid] = ['focusNeighbor'])
