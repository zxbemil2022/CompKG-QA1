<template>
  <div class="query-section" :class="{ collapsed: !visible }" :style="style">
    ...
    <div class="query-content content" v-show="visible">
      ...
      <div class="query-results" v-if="queryResult.answer || queryResult.error">
        <div class="answer-text">{{ queryResult.error || queryResult.answer }}</div>
        <div v-if="queryResult.citations?.length" class="citations">
          <div class="citations-title">【参考来源】</div>
          <ol>
            <li v-for="(item, idx) in queryResult.citations" :key="`${item}-${idx}`">{{ item }}</li>
          </ol>
        </div>
      </div>
    </div>
...
const queryResult = ref({ answer: '', citations: [], error: '' });
...
const onQuery = async () => {
  ...
  try {
    ...
    queryResult.value = {
      ...
      error: data?.status === 'failed' ? (data?.message || '检索失败') : '',
    };
  } catch (error) {
    console.error(error);
    message.error(error.message);
    queryResult.value = { answer: '', citations: [], error: error.message || '检索失败' };
  } finally {
