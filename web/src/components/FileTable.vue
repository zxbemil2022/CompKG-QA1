      ...
      <template #bodyCell="{ column, text, record }">
        ...
        <div v-else-if="column.key === 'action'" style="display: flex; gap: 4px;">
          ...
          <a-button
            class="rebuild-btn"
            type="text"
            @click="handleRebuildGraph(record)"
            :disabled="lock || record.status === 'processing' || record.status === 'waiting' || !isLightragDatabase"
            :icon="h(ReloadOutlined)"
            title="重建图谱"
          />
...
const isLightragDatabase = computed(() => String(store.database?.kb_type || '').toLowerCase() === 'lightrag');
...
const handleRebuildGraph = (record) => {
  if (!record?.file_id) return;
  store.handleRebuildGraph(record.file_id);
};
