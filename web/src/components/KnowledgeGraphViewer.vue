...
const emptyGraphRebuildTriggered = ref({})
...
const loadGraphData = async () => {
  ...
  try {
    ...
    if (graphResponse.success && statsResponse.success) {
      ...
      const diagnostics = graphResponse.data?.diagnostics || statsResponse.data?.diagnostics || {}
      if (graphResponse.data.nodes.length === 0 && !emptyGraphRebuildTriggered.value[selectedDatabase.value]) {
        emptyGraphRebuildTriggered.value[selectedDatabase.value] = true
        try {
          const rebuildResp = await lightragApi.rebuildIfEmpty(selectedDatabase.value, false)
          if (rebuildResp.success && ['queued', 'already_queued'].includes(rebuildResp.status)) {
            message.info('图谱为空，已触发一次自动补建任务，请稍后自动刷新')
            setTimeout(() => {
              loadGraphData()
            }, 2500)
          } else if (rebuildResp.success && rebuildResp.status === 'skipped' && diagnostics?.state) {
            message.warning(`图谱为空：${diagnostics.message || diagnostics.state}`)
          }
        } catch (rebuildError) {
          console.error('触发补建失败:', rebuildError)
          if (String(rebuildError?.message || '').includes('404')) {
            message.warning('当前后端未提供补建接口，请更新后端后重试')
          }
        }
      }
