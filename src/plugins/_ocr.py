...
class OCRPlugin:
    ...
    def __init__(self, **kwargs):
        ...
        self.model_dir_root = self._resolve_model_dir_root()

    def _resolve_model_dir_root(self) -> str:
        """解析模型根目录，优先环境变量，缺失时尝试常见目录。"""
        env_dir = os.getenv("MODEL_DIR") if not os.getenv("RUNNING_IN_DOCKER") else os.getenv("MODEL_DIR_IN_DOCKER")
        if env_dir:
            return env_dir

        candidates = [
            os.path.join(os.getcwd(), "models"),
            os.path.join(os.getcwd(), "saves", "models"),
        ]
        for candidate in candidates:
            if os.path.exists(candidate):
                logger.info(f"MODEL_DIR 未配置，自动使用: {candidate}")
                return candidate

        # 默认返回工作目录下 models，便于报错信息更可读
        return candidates[0]
        ...
        try:
            if not self.model_dir_root:
                raise OCRServiceException(
                    "未找到 MODEL_DIR，请设置 MODEL_DIR（或 Docker 下 MODEL_DIR_IN_DOCKER）指向 OCR 模型根目录",
                    "rapid_ocr",
                    "model_dir_missing",
                )
    ...
    def process_image(self, image, params=None):
        ...
        try:
            ...
            # 优先 RapidOCR，本地模型缺失时可回退到 PaddleX（可配置）
            if self.ocr is None:
                try:
                    self.load_model()
                except OCRServiceException as e:
                    fallback_provider = str(
                        params.get("ocr_fallback_provider")
                        or os.getenv("OCR_IMAGE_FALLBACK_PROVIDER", "none")
                    ).lower()
                    can_fallback = fallback_provider in {"paddlex", "paddle", "paddlex_ocr"}
                    if can_fallback and e.status_code in {"model_dir_missing", "model_not_found", "model_incomplete"}:
                        logger.warning(f"RapidOCR不可用，回退到 PaddleX 图像OCR: {e}")
                        return self._process_image_with_paddlex(image_path)
                    raise
    ...
    def _process_image_with_paddlex(self, image_path: str) -> str:
        """PaddleX 图像 OCR 回退通道。"""
        from .paddlex import analyze_document, check_paddlex_health

        paddlex_uri = os.getenv("PADDLEX_URI", "http://localhost:8080")
        health_check = check_paddlex_health(paddlex_uri)
        if not health_check.ok:
            detail = health_check.text
            raise OCRServiceException(
        f"PaddleX OCR服务健康检查失败: {detail}", "paddlex_ocr", "health_check_failed"
                )

        start_time = time.time()
        result = analyze_document(image_path, base_url=paddlex_uri)
        processing_time = time.time() - start_time
        if not result.get("success"):
            error_msg = f"PaddleX OCR图像处理失败: {result.get('error', 'unknown error')}"
            log_ocr_request("paddlex_ocr", image_path, False, processing_time, error_msg)
            raise OCRServiceException(error_msg, "paddlex_ocr", "processing_failed")

        text = str(result.get("full_text", "") or "")
        log_ocr_request("paddlex_ocr", image_path, True, processing_time)
        return text
