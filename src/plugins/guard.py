...
class ContentGuard:
    ...
    async def check_with_llm(self, text: str) -> bool:
        ...
        logger.debug(f"LLM response: {raw_reply}")

        # 仅接受“合规/不合规”作为首个结论，避免“并非不合规”等语义歧义导致误杀
        first_line = raw_reply.splitlines()[0].strip() if raw_reply else ""
        normalized = re.sub(r"[。.!！?？\\s]+", "", first_line)
        if normalized.startswith("不合规"):
            return True
        if normalized.startswith("合规"):
            return False

        # 兜底：若模型未按格式返回，默认按“合规”处理并记录告警
        logger.warning("Content guard LLM returned unexpected format, treat as compliant")
        return False
