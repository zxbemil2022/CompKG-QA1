...
async def process_file_to_markdown(file_path: str, params: dict | None = None) -> str:
    ...
    if file_ext == ".pdf":
        ...
        # 处理 JSON 文件：优先转为“可抽取语义文本”，并保留原始 JSON 代码块
        with open(file_path_obj, encoding="utf-8") as f:
            data = json.load(f)
        # 将 JSON 数据格式化为 markdown 代码块
        def _flatten_json(obj, prefix: str = "", out: list[str] | None = None):
            if out is None:
                out = []
        ...
        if isinstance(data, dict):
            ...
            if isinstance(docs, list):
                for i, doc in enumerate(docs, 1):
                    ...
                    if title:
                        semantic_lines.append(f"文档 {doc_id} 标题：{title}")
                    if text:
                        semantic_lines.append(f"文档 {doc_id} 内容：{str(text).strip()}")

            triples = data.get("triples") or data.get("relations")
            if isinstance(triples, list):
                for triple in triples:
                    ...
                    if head and rel and tail:
                        semantic_lines.append(f"{head} -> {rel} -> {tail}")

        if not semantic_lines:
            semantic_lines = _flatten_json(data)[:2000]

        semantic_block = "\n".join(f"- {line}" for line in semantic_lines if str(line).strip())
