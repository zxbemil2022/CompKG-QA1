...
def validate_img_embedding_file(file_path: str) -> bool:
    """
     校验是否为可用于图文嵌入的通用JSON文件。
     兼容旧版知识库图片结构，同时支持计算机知识结构。
     """
    # 校验文件格式
    ...
    text_keys = {"title", "name", "topic", "subject", "description", "definition", "content"}

    for item in json_content:
        if not isinstance(item, dict):
            return False
        # 至少要有一个文本类字段，便于构建文本/图文向量
        has_text_field = any(
        ...
        # image_url/detail_url/source_url 若出现，需为字符串且URL格式正确
        for url_field in ("image_url", "detail_url", "source_url"):
        ...
        # assets 若出现，支持 [{"type":"image","url":"..."}] 格式
